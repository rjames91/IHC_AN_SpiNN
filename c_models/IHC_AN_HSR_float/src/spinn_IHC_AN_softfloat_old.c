...
void app_init(void)
{
	...
	preSyn.CaTh[0]=0.2924207;//pow(1.43e-13,preSyn.power);
	preSyn.CaTh[1]=0.2924207;//pow(1.43e-13,preSyn.power);
	...
	Synapse.M[0]=REAL_CONST(8.);
	Synapse.M[1]=REAL_CONST(8.);
	  ...
	  start_count_process = tc[T2_COUNT];
		...
		for (j=0;j<NUMHSR;j++)
		{
			...
			if(i%resamp_fac==0)
			{
				...
				vrrhsr=REAL_CONST(100.)*compare;//to allow for single float range
				//saturate vrr
				if(vrrhsr>max_rate)
				{
					vrrhsr=max_rate;
				...
				//Probability=1-pow(1-releaseProb,ANAvailHSR[j]);
				if (refrac_hsr[j]>0)
				{
				...
				else if(Probability>((REAL)random_gen()/r_max))
				{
					...
					//refrac_hsr[j]=round(Synapse.refrac_period/dt + (((REAL)random_gen()/r_max)*Synapse.refrac_period)/dt);
					refrac_hsr[j]=(uint)(Synapse.refrac_period/dt_spikes + (((REAL)random_gen()/r_max)*Synapse.refrac_period)/dt_spikes);
				...
				{	
					...
					Synapse_xpow=Synapse_xpow*(REAL_CONST(1.)-(ANReproHSR[j]*Synapse.x*dt_spikes));
				}
				...
				reuptakeandlost=((Synapse.r*dt_spikes)+(Synapse.l*dt_spikes))*ANCleftHSR[j];
				reuptake=Synapse.r*dt_spikes*ANCleftHSR[j];
			...
			//TODO: switch this to DMA bursts?
			//out_buffer[((j)*SEGSIZE)+i]  = vrrhsr;
