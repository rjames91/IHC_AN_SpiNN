...
void app_init(void)
{
	...
	preSyn.CaTh[0]=1e-4;//pow(1e-14,preSyn.power);
	preSyn.CaTh[1]=1e-4;//pow(1e-14,preSyn.power);
		...
		for (j=0;j<NUMLSR;j++)
		{
			...
			if(i%resamp_fac==0)
			{
				...
				else if(Probability>((REAL)random_gen()/r_max))
				{
					...
					refrac_lsr[j]=(uint)(Synapse.refrac_period/dt_spikes + (((REAL)random_gen()/r_max)*Synapse.refrac_period)/dt_spikes);
				}
				else
				{
					ejected=REAL_CONST(0.);
				...
				for(k=0;k<M_q;k++)
				{			
					Synapse_xpow=Synapse_xpow*(REAL_CONST(1.)-(ANReproLSR[j]*Synapse.x*dt_spikes));
				...
				reuptakeandlost=((Synapse.r*dt_spikes)+(Synapse.l*dt_spikes))*ANCleftLSR[j];
				reuptake=Synapse.r*dt_spikes*ANCleftLSR[j];
