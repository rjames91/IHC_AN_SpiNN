...
void app_init(void)
{
    ...
    log_info("total ticks=%d\n",TOTAL_TICKS);
    log_info("data_size=%d\n",data_size);


    Fs=(REAL)sampling_freq;
    log_info("sampling freq=%d",(uint)Fs);
    ...
    log_info("resamp fac=%d",resamp_fac);
...
void data_read(uint null_a, uint null_b)
{
  ...
  start_count_read = tc[T2_COUNT];
	...
	else if (read_ticks >= TOTAL_TICKS)
	{
        //send finished processing MC packet
        log_info("total ticks reached, scheduling application end");
...
void c_main()
{
  ...
  //reads from DMA to DTCM every timer tick
